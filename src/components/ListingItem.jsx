  ...
  return (
    ...
    transition-shadow duration-150  m-[10px]'>
      <Link to={`/category/${listing.type}/${id}`}>
       ...
       transition-scale duration-200 ease-in" loading='lazy' src={listing.imgUrls[0]} alt=''/>
        <Moment className="absolute top-2 left-4 bg-[#3377cc] text-white uppercase text-xs font-semibold
        ...
        <div className='w-full p-[10px]'>
          <div className='flex items-center space-x-1'> 
            ...
            <p className='font-semibold text-sm mb-2 text-gray-600 w-[200px] truncate'>{listing.address}</p>
          </div>
          <p className='font-semibold text-xl m-0 w-[200px] truncate '> {listing.name}</p>
