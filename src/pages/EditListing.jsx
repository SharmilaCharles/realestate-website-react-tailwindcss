...
import { doc,updateDoc, docRef, addDoc, getDoc, collection} from 'firebase/firestore';
...
export default function EditListing() {
   ...
   const { type,name,bedrooms,bathrooms,
    parking,furnished,address,description,offer,
    regularPrice, discountedPrice, longitude, latitude,images} = formData
  ...
  async function onSubmit(e) {
    ...
    if (images.length > 6) {
        setLoading(false);
        toast.error("Maximum 6 images only allowed");
        return;
    }
    ...
    if(geolocationEnabled) {
        ...
        location = data.status === "ZERO_RESULTS" && !data.results[0];
  ...
  return (
    <main className='max-w-lg px-5  mx-auto bg-blue-50'> 
        <h1 className='text-3xl text-center mt-6 
        font-bold '> Update the Listing</h1>
        <form onSubmit={onSubmit} >
            <p className='text-lg mt-6 font-semibold'> Sell / Rent</p>
            <div className='flex'> 
                <button type='button' id="type" value="sale" 
                onClick={onChange} 
                className={`mx-3 px-7 py-3 font-medium text-sm uppercase shadow-sm
                rounded hover:shadow-lg focus:shadow-lg
                active:shadow-lg transition duration-500 ease-in-out w-full
                ${ type ==='rent' ? "bg-white" : "bg-blue-300"}`}>  
                    Sell
                </button>
                <button type='button' id="type" value="rent" 
                onClick={onChange} 
                className={`ml-3 px-7 py-3 font-medium text-sm uppercase shadow-sm
                rounded hover:shadow-lg focus:shadow-lg
                active:shadow-lg transition duration-500 ease-in-out w-full
                ${ type ==='sale' ? "bg-white" : "bg-blue-300"}`}>  
                    Rent
                </button> 
            </div>
                ...
                duration-100 ease-in-out focus:text-gray-700 focus:bg-white
                ...
                <div className='flex space-x-6 mb-6'> 
                    <div className=''>
                        ...
                        onChange={onChange} min="1" max="10"  required
            ...
            <div className='flex'> 
                ...
                rounded hover:shadow-lg focus:shadow-lg
                active:shadow-lg transition duration-500 ease-in-out w-full
                ${ !furnished ? "bg-white" : "bg-blue-300"}`}>  
                    Yes
                </button>
        ...
        <div className='mb-6'> 
            <p className='text-lg mb-2 font-semibold'>Images</p>
            <p className='text-gray-600 mb-2'>The first image will be cover(max 6)</p>
        ...
        py-3 bg-blue-600 text-white
        font-medium text-sm uppercase shadow-md 
        hover:bg-blue-600 hover:shadow-lg focus:bg-blue-700
        focus:shwdow-lg active:bg-blue-800 active:shadow-lg
        transition duration-150 ease-in-out' >Update Listing</button>
        </form>
